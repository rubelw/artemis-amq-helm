...
if [ "$LOG_FORMATTER" = "JSON" ]; then
    ...
    cp ../etc/logging.properties /tmp/logging.properties
    sed -i "s/handler.CONSOLE.formatter=.*/handler.CONSOLE.formatter=JSON/g" /tmp/logging.properties
    yes | cp /tmp/logging.properties ../etc/logging.properties

    #sed -i "s/handler.CONSOLE.formatter=.*/handler.CONSOLE.formatter=JSON/g" ../etc/logging.properties
fi

cp ../etc/logging.properties /tmp/logging.properties2
sed -i "s/logger.org.eclipse.jetty.level=WARN/logger.org.eclipse.jetty.level=DEBUG/g" /tmp/logging.properties2
yes | cp /tmp/logging.properties2 ../etc/logging.properties
...
if [ "$ARTEMIS_USERNAME" ] && [ "$ARTEMIS_PASSWORD" ]; then
  ...
  #cp ../etc/artemis-roles.properties /tmp/artemis-roles.properties
  #sed -i "s/amq[ ]*=.*/amq=$ARTEMIS_USERNAME\\n/g" /tmp/artemis-roles.properties
  echo "amq=${ARTEMIS_USERNAME}\\n" > ../etc//artemis-roles.properties
  #yes | cp /tmp/artemis-roles.properties ../etc/artemis-roles.properties
  ...
  echo "${ARTEMIS_USERNAME}=ENC(${HASHED_PASSWORD})\\n" > ../etc/artemis-users.properties
  #cp .A./etc/artemis-users.properties /tmp/artemis-users.properties
  #sed -i "s/artemis[ ]*=.*/$ARTEMIS_USERNAME=ENC($HASHED_PASSWORD)\\n/g" /tmp/artemis-users.properties
  #yes | cp /tmp/artemis-users.properties ../etc/artemis-users.properties

  #sed -i "s/artemis[ ]*=.*/$ARTEMIS_USERNAME=ENC($HASHED_PASSWORD)\\n/g" ../etc/artemis-users.properties

  #$BROKER_HOME/bin/artemis user add --user artemis --password artemis --role amq
...
echo "Add BROKER_CONFIGS env variable to startup options"
prepend_java_arg "BROKER_CONFIGS" "\$BROKER_CONFIGS"

echo "Loop through all BROKER_CONFIG_... and convert to java system properties"
